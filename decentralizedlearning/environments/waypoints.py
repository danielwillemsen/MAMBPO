...
class WaypointsEnv():
    def __init__(self, n=3):
        ...
        self.observation_space = [spaces.Box(low=self.max_o*0, high=self.max_o, shape=(2*n+4,)) for i in range(n)]       
        self.agents = [Agent() for agent in range(n)]
        self.targets = [Target() for target in range(n)]
