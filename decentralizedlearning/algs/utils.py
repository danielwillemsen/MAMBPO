...
class MultiStepReplayBuffer:
    def __init__(self, size=50000, batch_size=128, device="cpu", length=2):
    ...
    def add_mini_traj(self, o, a, r, done):
        o_temp = torch.stack(o[:-1])
        o_next_temp = torch.stack(o[1:])
        a_temp = torch.stack(a)
        r_temp = torch.stack(r)
        done_temp = torch.stack(done)
        self.add((o_temp, a_temp, r_temp, o_next_temp, done_temp))
    ...
    def initialize(self, sample):
        if not self.initialized:
            self.o = torch.zeros((self.max_size, self.length, sample[0].shape[-1]), device=self.device, dtype=torch.float32)
            self.a = torch.zeros((self.max_size, self.length, sample[1].shape[-1]), device=self.device, dtype=torch.float32)
            self.r = torch.zeros((self.max_size, self.length), device=self.device, dtype=torch.float32)
            self.o_next = torch.zeros((self.max_size, self.length, sample[3].shape[-1]), device=self.device, dtype=torch.float32)
            self.done = torch.zeros((self.max_size, self.length), device=self.device, dtype=torch.float32)
