...
def run_episode(env, agents, eval=False, render=False, generate_val_data=False, greedy_eval=True, steps=1000, store_data=False):
    ...
    for i in range(steps):
        ...
        for j, agent in enumerate(agents):
            ...
            act_n.append(action)
        # step environment
        obs_n, reward_n, done_n, _, reward_details = env.step(act_n)
...
def train(env, agents, data_log, n_episodes=10000, n_steps=None, generate_val_data=False, record_env=None):
    ...
    for i in ep_generator:
        ...
        score, step, extra_data = run_episode(env, agents, render=False, store_data=True)
        step_tot += step
    ...
    if True:
        for run in range(n_runs):
            ...
            for n_agent in [4]:
                ...
                env = EnvWrapper("custom", name, n_agents=n_agent, randomized=True)
                agent_fn = SAC
                algname = "SAC"
