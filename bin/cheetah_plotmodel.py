...
def train(env, agents, n_episodes=10000, n_steps=None, generate_val_data=False, record_env=None):
    ...
    for i in ep_generator:
        ...
        if i % 10 == 0:
            generate_statistics(agents, record_env)
...
def generate_statistics(agents, record_env):
...
def single_run(env, agent_fn, logdata, seed, agent_kwargs=dict(), n_episodes=None, n_steps=None, record_env=None):
    ...
    logdata[name].append(
        train(env, agents, n_episodes=n_episodes, n_steps=n_steps, generate_val_data=True, record_env=record_env))
    # env.close()
    ...
    if True:
        for run in range(n_runs):
            ...
            for steps in [40]:
                ...
                record_env = EnvWrapper("gym", name,
