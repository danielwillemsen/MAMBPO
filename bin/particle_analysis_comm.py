...
def train(env, agents, data_log, n_episodes=10000, n_steps=None, generate_val_data=False, record_env=None, trainer=None):
    ...
    for i in ep_generator:
        ...
        score, step = run_episode(env, agents, render=False, store_data=False, trainer=trainer)
        step_tot += step
        ...
        if i%250 == 249:
            ...
            for i_23 in range(10):
                score, step = run_episode(env, agents, render=False, eval=True, store_data=False, trainer=trainer)
            scores_greedy = []
            cols = []
            dists = []
            for e in range(50):
                score_greedy, _  = run_episode(env, agents, render=False, eval=True, store_data=False, trainer=trainer)
    ...
    if True:
        for run in range(n_runs):
            ...
            for n_agent in [4]:
                ...
                par = get_hyperpar("COM", alg=algname)
